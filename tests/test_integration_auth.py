...
def test_repeat_signup(client, monkeypatch):
    ...
    data = response.json()
    assert data["detail"] == messages.USER_EMAIL_OR_NAME_ALREADY_EXISTS


def test_signup_fail(client, monkeypatch):
    mock_send_email = Mock()
    monkeypatch.setattr("src.api.auth.send_email", mock_send_email)
    response = client.post("api/auth/register", json=user_data)
    assert response.status_code == 409, response.text
...
def test_request_email(client, monkeypatch):
    ...
    data = response.json()
    assert data["message"] == messages.EMAIL_SENT
...
def test_confirm_email_fail(client, get_email_token):
    response = client.get(f"api/auth/confirm-email/{get_email_token}")
    assert response.status_code == 200, response.text
    data = response.json()
    assert data["message"] == messages.USER_ALREADY_CONFIRMED
...
def test_confirm_email_wrong_token(client, get_email_token):
    ...
    assert response.status_code == 422, response.text
    data = response.json()
    assert data["detail"] == messages.UNEXISTING_TOKEN
...
def test_request_email_fail(client, monkeypatch):
    mock_send_email = Mock()
    monkeypatch.setattr("src.api.auth.send_email", mock_send_email)
    response = client.post(
        "api/auth/request_email", json={"email": "unexisting@test.com"}
    )
